...
class BaseBoostingAssembler(ModelAssembler):
    ...
    def __init__(self, model, trees, base_score=0):
        ...
        elif model.__module__ == 'lightgbm.basic' \
                and type(model).__name__ == 'Booster':
            model_dump = model.dump_model()
            objective = model_dump.get('objective', None)
            if objective is None:
                raise EnvironmentError('Please use a model generated by \
                    lightgbm which version >= 2.2.3')
            elif objective.startswith('binary') \
                    or objective.startswith('multiclass'):
                self._is_classification = True
                self._output_size = model_dump['num_class']

...
class JavaGenerator(BaseInterpreter):
    ...
    def interpret_if_expr(self, expr):
        var_name = self._add_var_declaration()

        if_def = "if (" + self._do_interpret(expr.test) + ") {"
        body_def = var_name + " = " + self._do_interpret(expr.body) + ";"
        else_def = "} else {"
        else_body = var_name + " = " + self._do_interpret(expr.orelse) + ";"

        self._add_code_line(if_def)
        self._current_indent += self._indent
        self._add_code_line(body_def)
        self._current_indent -= self._indent
        self._add_code_line(else_def)
        self._current_indent += self._indent
        self._add_code_line(else_body)
        self._add_closing_bracket()
    ...
    def _add_var_declaration(self, var_type="double"):
